...
def scanline_convert(polygons, i, screen, zbuffer):
  # mixing colors like home depot
  color = []
  color.append(random.randint(0, 255))
  color.append(random.randint(0, 255))
  color.append(random.randint(0, 255))

  # getting the coords from polygons
  x0 = polygons[i][0]
  y0 = polygons[i][1]
  z0 = polygons[i][2]
  ...
  # making sure points are in order, no funny business here
  if (y0 > y2):
    temp = x2
    x2 = x0
    x0 = temp
  ...
  # initializing the moving points in 3...2...1...
  y = y0
  xi = x0
  xf = x0
  zi = z0
  ...
  # getting the triangle$$$ (deltas, not the triangle polygons)
  delta_xi = (x2 - x0) / (y2 - y0)
  delta_zi = (z2 - z0) / (y2 - y0)

  # lets cook up some solids!!! (but just half of it though)
  if ((y1 - y0) != 0):
  ...
  # marinating the other half of the solids
  if ((y2 - y1) != 0):
